...
import com.springboot.netease.model.msg.MsgTypeEnum;
import lombok.Data;
...
public class SendMsgParam implements Serializable{
     ...
     * 可选，消息丢弃的概率。取值范围[0-9999]；
     ...
     此参数可用于流控特定业务类型的消息。
     */
    private String abandonRatio;
    /**
     *可选，true表示是高优先级消息，云信会优先保障投递这部分消息；false表示低优先级消息。默认false。
     *强烈建议应用恰当选择参数，以便在必要时，优先保障应用内的高优先级消息的投递。
     *若全部设置为高优先级，则等于没有设置。 高优先级消息可以设置进入后重发，见needHighPriorityMsgResend参数
     */
    private Boolean highPriority;
    /**
     * 可选，true表示会重发消息，false表示不会重发消息。默认true。
     * 注:若设置为true， 用户离开聊天室之后重新加入聊天室，在有效期内还是会收到发送的这条消息，目前有效期默认30s。
